...
export default class BlogClient {
  ...
  getBlogTopics(): Promise<Maybe<TopicItem[]>> {
    return this.blogCategoryId().then((MblogCategoryId) => {
      if (MblogCategoryId.isSome()) {
        return this.fetch<CategoryResponse>(
          `c/${MblogCategoryId.valueOrThrow()}.json`
        )
          .then((categoryResponse) => categoryResponse.topic_list.topics)
          .then((topics) =>
            topics.filter((t) => !t.tags?.includes(this.DISCOURSE_ONLY_TAG))
          )
          .then((topics) =>
            topics.sort((t1, t2) => t2.created_at.localeCompare(t1.created_at))
          )
  ...
  getFirstPostForTopic(topicId: number): Promise<Post> {
    return this.fetch<TopicPostsResponse>(`t/${topicId}/posts.json`).then((r) =>
      r.post_stream.posts.find((p) => p.post_number === 1)
    );
  }

  getTopicAndPostBySlug(slug: string): Promise<TopicAndFirstPost> {
    return this.fetch<TopicResponse>(`t/${slug}.json`).then((topic) => ({
      topic,
  ...
  async getAllBlogPostSlugs(): Promise<string[]> {
    const blogTopics = (await this.getBlogTopics()).valueOrThrow(
      'Could not get blog topics'
    );
